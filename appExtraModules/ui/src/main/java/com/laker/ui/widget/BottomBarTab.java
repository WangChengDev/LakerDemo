...
//    }
    ...
    private ImageView mIcon;
    private TextView mTvUnderIcon;
    private boolean showTvUnderIcon = false;
    private Context mContext;
    private int mTabPosition = -1;
    ...
    private void init(Context context, int icon, CharSequence title) {
        ...
        if (title != null) {
            showTvUnderIcon = true;
            mTvUnderIcon = new TextView(context);
            mTvUnderIcon.setText(title);
            ...
            mTvUnderIcon.setTextSize(10);
            mTvUnderIcon.setTextColor(ContextCompat.getColor(context, R.color.tab_unselect));
            mTvUnderIcon.setLayoutParams(paramsTv);
            lLContainer.addView(mTvUnderIcon);
        } else {
            showTvUnderIcon = false;
        }
//        mTvUnderIcon = new TextView(context);
//        mTvUnderIcon.setText(title);
//        LinearLayout.LayoutParams paramsTv = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
...
//        lLContainer.addView(mTvUnderIcon);
    ...
    public void setSelected(boolean selected) {
        ...
        if (selected) {
            mIcon.setColorFilter(ContextCompat.getColor(mContext, R.color.colorPrimary));
            if (showTvUnderIcon) {
                mTvUnderIcon.setTextColor(ContextCompat.getColor(mContext, R.color.colorPrimary));
            }
        } else {
            mIcon.setColorFilter(ContextCompat.getColor(mContext, R.color.tab_unselect));
            if (showTvUnderIcon) {
                mTvUnderIcon.setTextColor(ContextCompat.getColor(mContext, R.color.tab_unselect));
            }
        }
    }
